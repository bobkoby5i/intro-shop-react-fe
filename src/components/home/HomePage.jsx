...
class HomePage extends React.Component {
    constructor(props){
        ...
        this.state = {
          products: [], // tu zaciagniemy produkty ktore pokazujemy
          errorMessage: '',
          infoMessage: '',
    ...
    componentDidMount() {
        UserService.currentUser.subscribe(data =>{ // from observable. when loaded. 
          this.setState({
            currentUser: data
          })
        });
        ...
        UserService.findAllProducts().
          then(products => {
            this.setState({products: products.data});  // jednorazowo zaczytamy produkty
          });
    }
    ...
    purchaseProduct(product) {
        ...
        const transaction = new Transaction(this.state.currentUser, product); // no date no id.

        UserService.purchaseProduct(transaction)
          .then(data => {
            this.setState({infoMessage: "Transaction completed."});
          },error => {
